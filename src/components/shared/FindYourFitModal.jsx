...
export default function FindYourFitModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative z-10 bg-[#F5EDE0] w-full h-full md:h-auto md:max-h-[90vh] md:max-w-2xl md:rounded-sm overflow-y-auto">
        {/* Close button */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 md:top-6 md:right-6 z-20 text-[#5C1F2E]/60 hover:text-[#5C1F2E] transition-colors"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="px-6 md:px-12 pt-12 pb-10 md:pt-14 md:pb-12">
          {/* Progress bar */}
          {isQuestion && (
            <div className="mb-10">
          ...
          <AnimatePresence mode="wait">
            ...
            {isResult && resultData && (
              ...
              >
                ...
                <h2 className="font-serif text-2xl md:text-4xl leading-[1.15] text-[#5C1F2E] mb-8 pr-8">
                  {resultData.heading}
                </h2>
                <p className="text-[14px] md:text-[15px] text-[#5C1F2E]/75 leading-relaxed font-sans mb-10">
                  {resultData.body}
                </p>
                <div className="border-t border-[#5C1F2E]/15 pt-10 mb-10">
                  <p className="font-serif italic text-lg md:text-2xl text-[#5C1F2E] mb-5">
                    {resultData.subheading}
                  </p>
                  <p className="text-[14px] md:text-[15px] text-[#5C1F2E]/75 leading-relaxed font-sans">
                ...
                <div className="flex flex-col gap-4">
                  <a
                    ...
                    className="inline-flex items-center gap-3 bg-[#5C1F2E] text-[#F5EDE0] px-7 py-4 md:px-8 md:py-5 text-[11px] tracking-[0.18em] uppercase font-sans hover:bg-[#3d1420] transition-all duration-200 self-start"
                  >
                    {resultData.ctaLabel}
                    <span>→</span>
                  </a>
