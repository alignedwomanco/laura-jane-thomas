...
function buildPdf(report) {
  ...
  sections.forEach((section) => {
    ...
    entries.forEach(([key, value], idx) => {
      ...
      let answer = value;
      if (Array.isArray(value)) answer = value.join(", ");
      if (typeof value === "object" && value !== null) {
        // marketingAssets.assets is an object of {has, detail}
        answer = Object.entries(value)
          .map(([k, v]) => {
            if (typeof v === "object" && v !== null) {
              return `${k}: ${v.has || ""} ${v.detail || ""}`.trim();
            }
            return `${k}: ${v}`;
          })
          .join("\n");
...
Deno.serve(async (req) => {
  try {
    ...
    console.error('[downloadSubmissionPDF] Error:', error.message);
